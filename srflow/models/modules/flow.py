...
class UnsqueezeLayer(nn.Module):
    ...
    def forward(self, input):
        return unsqueeze2d(input, self.factor)


class FusedConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels,
                 kernel_size=[3, 3], stride=[1, 1],
                 padding="same", do_actnorm=True, weight_std=0.05):
        padding = Conv2d.get_padding(padding, kernel_size, stride)
    ...
    def fuse_actnorm(self):
        ...
        self.weight.data = self.weight.data * torch.exp(logs)[:, None, None, None]
        self.bias = nn.Parameter(torch.exp(logs) * bias, requires_grad=self.weight.requires_grad)
        self.is_fused = True

    def forward(self, input):
        if self.do_actnorm and not self.is_fused:
            self.fuse_actnorm()
        x = super().forward(input)
        return x