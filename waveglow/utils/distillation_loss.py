...
class DistillationLoss(nn.Module):
    def __init__(self, student, teacher, *, infer_teacher: Optional[callable] = None,
                 teacher_dtype: torch.dtype, stft_loss_coeff: float, mel: bool = True, log: bool = True,
                 ...
                 eps=1e-5):
        """
        A pytorch module that computes a differentiable power loss in either normal or mel space.
        :type student: [Flow, WideFlow, Affine, WaveNet]Student
        :type teacher: WaveGlowTeacher
        :param infer_teacher: optionally provide a more efficient version of teacher.infer, see trace_waveglow_teacher
        ...
        self.teacher_dtype, self.stft_loss_coeff, self.mel, self.log = teacher_dtype, stft_loss_coeff, mel, log
    ...
    def forward(self, mel: torch.Tensor, audio=None, sigma=1.0):
        """
        ...
        with torch.no_grad():
            if audio is None:
                ...
                reference = audio
                student_input, upsampled_mels = self.teacher.flow_forward(mel, audio)
