...
class FlowUpsamplerNet(nn.Module):
    ...
    def forward(self, gt=None, rrdbResults=None, z=None, epses=None,  eps_std=None):
        epses_copy = [eps for eps in epses] if isinstance(epses, list) else epses
        sr = self.decode(rrdbResults, z, eps_std, epses=epses_copy)
        return sr

    def decode(self, rrdbResults, z, eps_std=None, epses=None):
        ...
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            ...
            if isinstance(layer, Split2d):
                fl_fea = self.forward_split2d_reverse(eps_std, epses, fl_fea, layer,
                                                      rrdbResults[self.levelToName[level]])
                # print('Split2d', fl_fea.shape)
            elif isinstance(layer, FlowStep):
                fl_fea = layer(fl_fea, rrdbResults=level_conditionals[level])
                # print('flowStep', fl_fea.shape)
            else:
                fl_fea = layer(fl_fea, reverse=True)
                # print('Other', fl_fea.shape)
        sr = fl_fea
        assert sr.shape[1] == 3
        return sr

    def forward_split2d_reverse(self, eps_std, epses, fl_fea, layer, rrdbResults):
        ft = None if layer.position is None else rrdbResults[layer.position]
        return layer(fl_fea, eps=epses.pop() if isinstance(epses, list) else None,
                       eps_std=eps_std, ft=ft)
